...
def load_features(datafile):
    ...
    d         = pd.read_csv(datafile, index_col = 0, parse_dates = True)
...
rain_feat, flow = load_features('dataframe.csv')
...
for epoch in range(epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'rrformer_weights.csv')
...
plt.savefig(os.path.join(figures_dir, 'rrformer_training_history.png'), dpi=150)
plt.show()
...
model = RRFormer(ninputs, d_model, nhead, num_enc_layers, num_dec_layers,
                 dim_feedforward, window_len, pred_len, dropout)
model.load_state_dict(torch.load('rrformer_weights.csv'))
model.eval()
...
for split, rain_sc, flow_raw, title in [
    ...
    plt.savefig(os.path.join(figures_dir, f'rrformer_{split}_predictions.png'), dpi=150)

...
## Mixed baseline:
#   Precip (0), Precip 30d (2), Precip 7d (3), Melt (6) → 0 (no forcing)
#   ETp (1), Temp (4), Groundwater (5) → training mean
train_mean = inputs_train.mean(dim=1, keepdim=True)       # (1, 1, 7)
baseline   = torch.zeros_like(inputs_val)                 # start from all zeros

#Set training mean for: ETp(1), temp(4), groundwater(5)
for col in [1, 4, 5]:
    baseline[:, :, col] = train_mean[:, :, col].expand_as(baseline[:, :, col])
baseline = baseline.detach()


feature_names = ['Precip', 'Etp', 'Precip 30d', 'Precip 7d', 'Temp', 'Groundwater', 'Melt']
...
colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#e377c2', '#7f7f7f','#17becf']

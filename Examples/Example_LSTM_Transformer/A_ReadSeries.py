...
rain['date']=pd.to_datetime(datetimestring,format='%Y-%m-%d %H:%M:%S')
...
rain=rain.groupby('date').sum(numeric_only = True)
...
rain=rain.resample('D').asfreq(fill_value=0)
...
refet['date']=pd.to_datetime(refet['date'],format='%d-%m-%Y %H:%M')
